...
- (void)addMagicSubviews
{
    ...
    if (!self.displayMenuBarIndependent) {
...
- (void)updateFrameForSubviews
{
    ...
    CGFloat topY = _againstStatusBar ? VTSTATUSBAR_HEIGHT : 0 + self.headerTopEdge;
    ...
    tipFrame.origin.y = CGRectGetMaxY(_navigationView.frame);
    tipFrame.origin.x = 0;
    tipFrame.size.width = size.width;
...
- (void)reloadData
{
    ...
    [currentItem.titleLabel setFont:[currentItem.titleLabel.font fontWithSize:self.selectedFontSize]];
}

- (void)preLoadAdjoiningPages {
    if (_contentView.currentPage > 0) {
        [_contentView creatViewControllerAtPage:_contentView.currentPage - 1];
    }
    if (_contentView.currentPage < _contentView.pageCount - 1) {
        [_contentView creatViewControllerAtPage:_contentView.currentPage - 1];
    }
}
...
- (__kindof UIViewController *)createViewControllerAtPage:(NSUInteger)pageIndex
{
    return [_contentView viewControllerAtPage:pageIndex autoCreate:YES];
}
...
- (void)graduallyChangeColor
{
    ...
    if (absScale > 0) {//change font size
        CGFloat fDelta = round((self.selectedFontSize - self.normalFontSize) * absScale);
        [currentItem.titleLabel setFont:[currentItem.titleLabel.font fontWithSize:(self.selectedFontSize - fDelta)]];
        [nextItem.titleLabel setFont:[nextItem.titleLabel.font fontWithSize:(self.normalFontSize + fDelta)]];
    }
...
- (void)resetMenuItemColor
{
    ...
    if (nextItem != currentItem)
        [nextItem.titleLabel setFont:[nextItem.titleLabel.font fontWithSize:self.normalFontSize]];
}
...
- (UIButton *)menuBar:(VTMenuBar *)menuBar menuItemAtIndex:(NSUInteger)index
{
    ...
    if (self.normalFontSize < 1)
        self.normalFontSize = catItem.titleLabel.font.pointSize;
    if (self.selectedFontSize < 1)
        self.selectedFontSize = catItem.titleLabel.font.pointSize;
    return catItem;
}
...
- (void)updateItemStateForDefaultStyle
{
    ...
    [catItem.titleLabel setFont:[catItem.titleLabel.font fontWithSize:self.normalFontSize]];
    [_menuBar.selectedItem.titleLabel setFont:[seletedItem.titleLabel.font fontWithSize:self.selectedFontSize]];
}
...
- (void)setHeaderTopEdge:(CGFloat)headerTopEdge {
    if (fabs(headerTopEdge - _headerTopEdge) < 0.01) {
        return;
    }
    _headerTopEdge = headerTopEdge;
    [self updateFrameForSubviews];
}

- (void)setHeaderTopEdge:(CGFloat)headerTopEdge duration:(CGFloat)duration {
    if (fabs(headerTopEdge - _headerTopEdge) < 0.01) {
        return;
    }
    _headerTopEdge = headerTopEdge;
...
- (void)setHeaderHidden:(BOOL)headerHidden againstStatusBar:(BOOL)againStatusBar duration:(CGFloat)duration
{
    _againstStatusBar = againStatusBar;
...
- (void)setSliderHeight:(CGFloat)sliderHeight {
    ...
    self.sliderView.layer.cornerRadius = sliderHeight / 2;
