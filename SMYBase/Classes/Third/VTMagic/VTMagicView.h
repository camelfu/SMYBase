...
/**
 *  顶部菜单与ContentView之前的TipView
 */
@property (nonatomic, strong, readwrite) UIView *tipView;
 ...
 *  容器视图
 */
@property (nonatomic, readonly) VTContentView *contentView;
...
@property (nonatomic, assign) BOOL displayMenuBarIndependent;

@property (nonatomic, readonly) VTMenuBar *menuBar; //顶部导航菜单视图

- (void)updateMenuBarState;

- (void)setHeaderTopEdge:(CGFloat)topEdge duration:(CGFloat)duration;

/**
 *  显示或隐藏头部组件
 *
 *  @param headerHidden 是否隐藏
    @param againStatusBar 是否紧贴系统状态栏
 *  @param duration     动画时长
 */
 ...
 *  头部的上边距
 */
@property (nonatomic, assign) CGFloat headerTopEdge;
